...
**Use `async` for independent third-party.** Analytics, A/B tooling, chat widgets — anything that doesn't depend on your code or DOM order. Always pair with [Subresource Integrity](../security/subresource-integrity.md) and `crossorigin="anonymous"` so a compromised CDN cannot ship modified code to your visitors:
